...
class CalendarEventList extends React.Component {
  ...
  static formatEventDate(event) {
    ...
    if ("date" in event.start && "date" in event.end) {
      ...
      if (start.valueOf() === end.valueOf()) {
        ...
        formattedDate = `${this.formatDateString(
          start
  ...
  static getDate(value) {
    return new Date(
      value.date !== undefined ? `${value.date}(CDT)` : value.dateTime
    );
  }
  ...
  componentDidMount() {
    ...
    let finalURL =
      "https://www.googleapis.com/calendar/v3/calendars/d5imtm374liirqn82loksf80s75episc@import.calendar.google.com/events?key=AIzaSyCR3-ptjHE-_douJsn8o20oRwkxt-zHStY";
    finalURL +=
      "&fields=items(summary,start,end,recurrence,htmlLink,description)"; // Only return the needed fields
